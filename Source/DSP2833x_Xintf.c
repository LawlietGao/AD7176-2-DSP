...
void InitXintf(void)
{
    ...
    // 
    XintfRegs.XTIMING0.bit.X2TIMING = 0;   //禁止双倍read/write lead/active/trail时序
    XintfRegs.XTIMING0.bit.USEREADY = 0;   //不采样XREADY输入
    XintfRegs.XTIMING0.bit.READYMODE = 1;  // 忽略

    // Size must be either:
    // 0,1 = x32 or
    // 1,1 = x16 other values are reserved
    XintfRegs.XTIMING0.bit.XSIZE = 3;      //32位数据总线模式
